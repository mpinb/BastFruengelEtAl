...
class Strategy(object):
    ...
    def set_split(self, split, setup = True):
        ...
        self.get_score = partial(self.get_score_static, self._get_score, cupy_split = cupy_split)
        self.get_y = partial(self.get_y_static, self.y, numpy_split = numpy_split)
        self._objective_function = partial(self._objective_function_static, self.get_score, self.get_y)
        if setup:
...
class DataExtractor_spatiotemporalSynapseActivation(DataExtractor):
    ...
    def setup(self, Rm):
        ...
        self.width = Rm.width
        self.selected_indices = Rm.selected_indices
        self._get_spatiotemporal_input()

    def _get_spatiotemporal_input(self):
        ...
        for m, single_mdb in enumerate(mdb):
            sa_path = single_mdb['binned_synapse_activation']
            sa = numpy.load(sa_path)['arr_0']
            if self.selected_indices is not None:
                sa = sa[:,:,self.selected_indices[m],self.tmax-self.width:self.tmax] 
            else:
                sa = sa[:,:,:,self.tmax-self.width:self.tmax] 
            saexc = sa[0].swapaxes(0, 1).swapaxes(1, 2)
            sainh = sa[1].swapaxes(0, 1).swapaxes(1, 2)
            self.data[('EXC',)].append(saexc)
            self.data[('INH',)].append(sainh)
            
        self.data = {k: numpy.concatenate(self.data[k], axis = 0) for k in self.data}
...
class Strategy_categorizedTemporalRaisedCosine(Strategy):
    ...
    def _setup(self):
        ...
        self._get_score = partial(self._get_score_static, self.base_vectors_arrays_dict)
...
class DataExtractor_spiketimes(DataExtractor):
    ...
    def _get_spike_times(self):
        ...
        for single_mdb in mdb:
            st = pd.read_csv(single_mdb['spike_times']).set_index('sim_trail_index')
            if self.selected_indices is not None:
                st_list.append(st.iloc[self.selected_indices[m]])
            else:
                st_list.append(st)
        self.data = pd.concat(st_list)
...
class Solver_COBYLA(Solver):
    ...
    def _setup(self):
        self.optimize = partial(self._optimize, self.strategy._objective_function, maxiter = 5000)
...
class Strategy_spatiotemporalRaisedCosine(Strategy):
    ...
    def normalize(self, x, flipkey = None):
        ...
        x_exc_z = x[('EXC',)][0]
        x_inh_z = x[('INH',)][0]
        norm_exc = b.get_superposition(x_exc_t)[numpy.argmax(numpy.abs(b.get_superposition(x_exc_t)))]
        norm_inh = -1*b.get_superposition(x_inh_t)[numpy.argmax(numpy.abs(b.get_superposition(x_inh_t)))]
        # spatial
        b = self.RaisedCosineBasis_spatial
        # norm_spatial = sum(np.abs(b.get_superposition(x_exc_z)) + np.abs(b.get_superposition(x_inh_z)))
        norm_spatial = max(numpy.abs(b.get_superposition(x_exc_z * norm_exc)))
    ...
    def visualize(self, optimizer_output, only_successful = False, normalize = False):
        fig = plt.figure(figsize = (10,5))
        ax_z = fig.add_subplot(1,2,1)
        ax_t = fig.add_subplot(1,2,2)
        for name, out in optimizer_output.iterrows():
            if only_successful:
                if not out.success:
                    continue
            if normalize:
            ...
            for group, (x_z, x_t) in dict_.iteritems():
                ...
                self.RaisedCosineBasis_temporal.visualize_x(convert_to_numpy(x_t), ax = ax_t, plot_kwargs = {'c': c})
                self.RaisedCosineBasis_spatial.visualize_x(convert_to_numpy(x_z), ax = ax_z, plot_kwargs = {'c': c})
...
class CombineStrategies_sum(Strategy):
    ...
    def _setup(self):
        ...
        self._get_score = partial(self._get_score_static, score_functions, self.lens)
